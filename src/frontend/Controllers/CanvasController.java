...
public class CanvasController extends AnchorPane
    ...
    private void handleArrowMenu(MouseEvent e) {
        ...
        e.consume();
        /*
         * List<Arrow> closest = new ArrayList<>(); double min = 10000; for (Arrow a :
         * arrows) { double distance = a.getDistaceFromClick(e); if(distance == min){
         * closest.add(a); } if (distance < min) { min = distance; closest.clear();
         * closest.add(a); } } if (min <= 15) { //trying to merge dragarrow into
         * existing arrow if (makingArrow) { ScaledPoint offset = new ScaledPoint
         * (Scale.Frontend,(int) (arrowStart.getX()-arrowBox.getLayoutX()),(int)
         * (arrowStart.getY()-arrowBox.getLayoutY()));
         * diagram.createRelation(arrowBox.getBox(),offset,arrowMap.get(closest.get(0)),
         * offset, ArrowType.ASSOCIATION); } else{ clickedArrow = closest;
         * arrowTypeComboBox.getSelectionModel().select(closest.get(0).getType());
         * openArrowMenu(e.getX(), e.getY()); } } if(makingArrow){ removeDragArrow(); }
         */
        e.consume();
    }
